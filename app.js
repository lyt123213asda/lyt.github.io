...
const templateUpdateResult = document.getElementById("templateUpdateResult");

// 最近一次“分年级按天统计”的结果缓存（用于导出）
let lastGradeStatsCache = null;
// 模板 Excel 缓存（上传一次，多次统计后再下载）
let templateWorkbook = null;
let templateBaseName = "";
let templateTotalUpdates = 0;
...
function bindEvents() {
  ...
  if (applyHoursToTemplateBtn) {
    applyHoursToTemplateBtn.addEventListener("click", onLoadTemplateWorkbook);
  }
  if (downloadTemplateBtn) {
    downloadTemplateBtn.addEventListener("click", onDownloadTemplateWorkbook);
  }
}
...
function hasHeaderKeyword(text, keyword) {
  return normalizeHeaderText(text).includes(keyword);
}
...
function findNameHeaderCell(matrix) {
  ...
  for (let r = 0; r < rows.length; r++) {
    const row = rows[r] || [];
    const rowText = normalizeHeaderText(row.join(""));
...
function findTotalActualHoursColumn(sheet, matrix) {
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    row.forEach((text, idx) => {
      if (hasHeaderKeyword(text, "总课时")) totalIdx = idx;
    });
...
function findTeachingActualHoursColumn(sheet, matrix) {
  const { rows, range } = matrix;
  const merges = sheet["!merges"] || [];
  let groupMerge = null;

  merges.forEach(m => {
    if (groupMerge) return;
    const cell = sheet[XLSX.utils.encode_cell(m.s)];
    const text = cell ? cell.v : "";
    if (hasHeaderKeyword(text, "上课")) {
      groupMerge = m;
    }
  });

  if (groupMerge) {
    const startRow = groupMerge.s.r - range.s.r;
    const searchRows = [startRow + 1, startRow + 2, startRow + 3];
    for (let c = groupMerge.s.c; c <= groupMerge.e.c; c++) {
      for (let i = 0; i < searchRows.length; i++) {
        const r = searchRows[i];
        if (r < 0 || r >= rows.length) continue;
        const text = (rows[r] || [])[c - range.s.c];
  ...
  // 兜底：同一行包含“上课”和“实上”，取距离“上课”最近的“实上”
  for (let r = 0; r < rows.length; r++) {
    const row = rows[r] || [];
    let teachIdx = -1;
    row.forEach((text, idx) => {
      if (hasHeaderKeyword(text, "上课")) teachIdx = idx;
    });
    if (teachIdx === -1) continue;
    let candidate = -1;
    row.forEach((text, idx) => {
      if (hasHeaderKeyword(text, "实上")) {
        if (candidate === -1) candidate = idx;
        else if (Math.abs(idx - teachIdx) < Math.abs(candidate - teachIdx)) {
          candidate = idx;
        }
      }
    });
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    for (let c = 0; c < row.length; c++) {
      const text = normalizeHeaderText(row[c]);
      if (text && text.includes("上课") && text.includes("实上")) {
        return range.s.c + c;
      }
    }
  }

  // 额外兜底：同一列的表头多行中出现“上课”和“实上”，且不包含“总课时”
  const headerScanRows = Math.min(rows.length, 8);
  for (let c = 0; c < (rows[0] || []).length; c++) {
    ...
    for (let r = 0; r < headerScanRows; r++) {
      const text = rows[r]?.[c];
      if (!text) continue;
      if (hasHeaderKeyword(text, "上课")) hasTeach = true;
      if (hasHeaderKeyword(text, "实上")) hasActual = true;
      if (hasHeaderKeyword(text, "总课时")) hasTotal = true;
    }
    if (hasTeach && hasActual && !hasTotal) {
      return range.s.c + c;
    }
  }
...
function onCalcGradeStats(e) {
  ...
  // 如果已加载模板，则把本次统计结果写入模板
  applyGradeStatsToTemplate(rows);
}
...
function onLoadTemplateWorkbook() {
  if (typeof XLSX === "undefined") {
    alert("Excel 解析库未加载成功，请检查网络后刷新页面重试。");
    return;
  }
  ...
  reader.onload = function (e) {
    try {
      const data = new Uint8Array(e.target.result);
      templateWorkbook = XLSX.read(data, { type: "array", cellStyles: true });
      templateBaseName = file.name.replace(/(\.xlsx?|\.xlsm)$/i, "");
      templateTotalUpdates = 0;
      if (downloadTemplateBtn) downloadTemplateBtn.disabled = false;
      if (templateUpdateResult) {
        templateUpdateResult.textContent = `模板已加载：${file.name}。之后每次统计会写入“上课-实上”，全部统计完成后点击“下载最终版本”。`;
      }
    } catch (err) {
...
function applyGradeStatsToTemplate(rows) {
  if (!templateWorkbook || !rows || !rows.length) return;
  ...
  templateWorkbook.SheetNames.forEach(sheetName => {
    ...
    const actualCol = findTeachingActualHoursColumn(sheet, matrix);
    if (actualCol === null || typeof actualCol === "undefined") return;
    foundActualColumn = true;
  ...
  if (templateUpdateResult) {
    if (!foundActualColumn) {
      templateUpdateResult.textContent =
        "未识别到模板中的“上课-实上”列，请检查模板格式。";
      return;
    }
    if (!hitCount) {
      templateUpdateResult.textContent =
    ...
    templateTotalUpdates += updatedCount;
    templateUpdateResult.textContent = `本次已更新 ${updatedCount} 行，累计更新 ${templateTotalUpdates} 行。`;
  }
}

function onDownloadTemplateWorkbook() {
  if (typeof XLSX === "undefined") {
    alert("Excel 导出库未加载成功，请检查网络后刷新页面重试。");
    return;
  }
  if (!templateWorkbook) {
    alert("请先上传模板 Excel，然后再下载最终版本。");
    return;
  }
  const baseName = templateBaseName || "老师课时";
  const fileName = `${baseName}_最终版.xlsx`;
